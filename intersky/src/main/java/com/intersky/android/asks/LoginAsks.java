...
public class LoginAsks {
    ...
    public static void doLoging(String account, String password, String szImei, String computername, Handler mHandler, Context mContext)
    {
        ...
        try {
            String urlString = NetUtils.getInstance().praseUrl(InterskyApplication.mApp.mService,LOGIN_PATH);//+"?"+"username="+account+"&password="+password;
...
//            AppUtils.showMessage(mContext,"lingk:"+urlString);
    ...
    public static void doLogout(Handler mHandler,Context mContext) {
        ...
        try {
            String urlString = NetUtils.getInstance().praseUrl(InterskyApplication.mApp.mService,EXIT_PATH);
            JSONObject jsonObject = new JSONObject();
    ...
    public static void doPushLogout(Handler mHandler,Context mContext) {
        ...
        String urlString = OaUtils.mOaUtils.createURLStringoa(InterskyApplication.mApp.mService,PUSH_PATH);
        ...
        items.add(item);
        item = new BasicNameValuePair("device_token", InterskyApplication.mApp.szImei);
        items.add(item);
    ...
    public static void doPushlogin(Handler mHandler,Context mContext) {
        if (InterskyApplication.mApp.deviceId.length() != 0) {
            ...
            item = new BasicNameValuePair("device_token", InterskyApplication.mApp.szImei);
            items.add(item);
            item = new BasicNameValuePair("device_id", InterskyApplication.mApp.deviceId);
            items.add(item);
    ...
    public static void safelogin(Handler mHandler,Context mContext,String serial,String type) {
        ...
        try {
            String urlString = NetUtils.getInstance().praseUrl(InterskyApplication.mApp.mService,LOGIN_PATH_SCAN);
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("serial",serial);
            jsonObject.put("type",type);
            jsonObject.put("user_id",InterskyApplication.mApp.mAccount.mRecordId);
            jsonObject.put("token", NetUtils.getInstance().token);
            String postBody =  jsonObject.toString();
