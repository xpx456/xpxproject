...
public class ContactManager {
    ...
    private void initCompanyContacts() {
        ...
        try {
            ...
            ContactsPrase.praseContacts(jo, mOrganization.organizationContacts);
    ...
    public void getPhoneContacts(BaseActivity activity)
    {

        LocalContactsHandler mLocalContactsHandler = new LocalContactsHandler(activity);
        activity.mBasePresenter.setPermissionsRequest(new GetPhoneContactsPremissionResult(activity,mLocalContactsHandler));
        AppUtils.getPermission(Manifest.permission.READ_CONTACTS,activity, PermissionCode.PERMISSION_REQUEST_READ_CONTACTS,mLocalContactsHandler);
    }
    ...
    public void praseUnderline() {
        try {
            ...
            for (int i = 0; i < users.length(); i++) {
                ...
                Contacts mContactModel2 = mOrganization.getContact(maildata.getString("userid"));
    ...
    public void sendFileIm(Context context,String path) {
        Contacts contactModel = new Contacts();
        contactModel.mId = "baseroot";
        contactModel.setName("联系人");
        contactModel.mHeadContacts.addAll(mContactManager.mOrganization.allContactsHead);
        contactModel.mMyContacts.addAll(mContactManager.mOrganization.allContacts);
        contactModel.mContacts.addAll(mContactManager.mOrganization.allClassContacts);
        mContactManager.mOrganization.showContacts = contactModel;
        Intent intent = new Intent(context,ContactsListActivity.class);
        intent.putExtra("onSelect",true);
        intent.putExtra("path",path);
        intent.setAction(ContactManager.ACTION_SEND_IM);
        context.startActivity(intent);
    }

    public void sendIm(Context context,String text) {
        Contacts contactModel = new Contacts();
        contactModel.mId = "baseroot";
        contactModel.setName("联系人");
        contactModel.mHeadContacts.addAll(mContactManager.mOrganization.allContactsHead);
    ...
    public void getContacts(String records, ArrayList<Contacts> mContactModels) {
        if (records.length() > 0) {
            ...
            for (int i = 0; i < strs.length; i++) {
                Contacts model = mContactManager.mOrganization.getContact(strs[i]);
