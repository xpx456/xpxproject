...
import intersky.appbase.PermissionDefine;
import intersky.appbase.PermissionItem;
...
import intersky.guide.view.activity.BaseActivity;
...
public class BasePresenter implements Presenter {
	...
	public void Create() {
		...
		ToolBarHelper.setSutColor(mBaseActivity, Color.parseColor("#00000000"));
	...
	public void getPermission(String permission,boolean finish) {
		if(PermissionDefine.percode.containsKey(permission))
		{
			if (ActivityCompat.checkSelfPermission(mBaseActivity, permission) != PackageManager.PERMISSION_GRANTED) {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
					mBaseActivity.permissionRepuestList.add(new PermissionItem(permission,mBaseActivity,finish));
			...
			if(permissions.size() > 0)
			{
				if(PermissionDefine.percode.containsKey(permissions.get(0)))
				{
					if (ActivityCompat.checkSelfPermission(mBaseActivity, permissions.get(0)) != PackageManager.PERMISSION_GRANTED) {
						if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
							mBaseActivity.permissionRepuestList.add(new PermissionItem(permissions.get(0),mBaseActivity,permissions,finish));
