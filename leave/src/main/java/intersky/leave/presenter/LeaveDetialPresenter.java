...
public class LeaveDetialPresenter implements Presenter {
	...
	public void initView() {
		...
		mLeaveDetialActivity.mCopyerLayer = (RelativeLayout) mLeaveDetialActivity.findViewById(R.id.cc);
		...
		mLeaveDetialActivity.head =  (TextView) mLeaveDetialActivity.findViewById(R.id.headimg);
		...
		mLeaveDetialActivity.copyer = (LinearLayout) mLeaveDetialActivity.findViewById(R.id.copyer);
		...
		mLeaveDetialActivity.more = mLeaveDetialActivity.findViewById(R.id.more);
		mLeaveDetialActivity.more.setOnClickListener(mLeaveDetialActivity.mMoreListenter);
		ImageView back = mLeaveDetialActivity.findViewById(R.id.back);
		back.setOnClickListener(mLeaveDetialActivity.mBackListener);
		TextView title = mLeaveDetialActivity.findViewById(R.id.title);
		title.setText(Bus.callData(mLeaveDetialActivity,"chat/getContactName",mLeaveDetialActivity.mLeave.uid)
				+ mLeaveDetialActivity.getString(R.string.xml_leave_type_name));
		AppUtils.setContactCycleHead(mLeaveDetialActivity.head, (String) Bus.callData(mLeaveDetialActivity,"chat/getContactName",mLeaveDetialActivity.mLeave.uid));
	...
	public void initselectView(ArrayList<Contacts> mselectitems, LinearLayout mlayer) {
		...
		if (mselectitems.size() > 0) {
			...
			for (int i = 0; i < mselectitems.size(); i++) {
				...
				View mView = mLeaveDetialActivity.getLayoutInflater().inflate(R.layout.task_contact_item, null);
	...
	public View.OnClickListener showHeadListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			Bus.callData(mLeaveDetialActivity,"chat/startContactDetial",v.getTag());
		}
	};
	...
	public void initDetial() {
		try {
			mLeaveDetialActivity.mBeginText.setText(mLeaveDetialActivity.getString(R.string.keyword_begin)+":"+mLeaveDetialActivity.mLeave.start.substring(0, 16));
			mLeaveDetialActivity.mEndText.setText(mLeaveDetialActivity.getString(R.string.keyword_end)+":"+mLeaveDetialActivity.mLeave.end.substring(0, 16));
			mLeaveDetialActivity.mTypeText.setText(mLeaveDetialActivity.getString(R.string.xml_leave_type_title)+":"+LeaveManager.getInstance().gettypeName(mLeaveDetialActivity.mLeave.leave_type_id));
			mLeaveDetialActivity.mCountText.setText(mLeaveDetialActivity.getString(R.string.xml_leave_day)+":"+mLeaveDetialActivity.mLeave.count);
			...
			if(LeaveManager.getInstance().oaUtils.mAccount.mRecordId.equals(mLeaveDetialActivity.mLeave.uid) && mLeaveDetialActivity.mLeave.is_approval != 2)
				mLeaveDetialActivity.more.setVisibility(View.VISIBLE);
			else
				mLeaveDetialActivity.more.setVisibility(View.INVISIBLE);
	...
	public void initApproveView() {
		...
		for(int i = 0 ; i < mLeaveDetialActivity.mApproves.size() ; i++)
		{
			...
			AppUtils.setContactCycleHead(mhead,mContact.getName());
				...
				AppUtils.setContactCycleHead(mhead,(String) Bus.callData(mLeaveDetialActivity,"chat/getContactName",mLeaveDetialActivity.mLeave.uid));
