...
public class FaceConversionUtil {
    ...
    private HashMap<String, String> emojiMapGif = new HashMap<String, String>();
    /** 保存于内存中的表情集合 */
    private List<ChatEmoji> emojisg = new ArrayList<ChatEmoji>();
    private List<ChatEmoji> emojis = new ArrayList<ChatEmoji>();
    /** 表情分页的结果集合 */
    public List<List<ChatEmoji>> emojiListsg = new ArrayList<List<ChatEmoji>>();
    ...
    public SpannableString addFace(Context context, int imgId,
        ...
        DisplayMetrics metric = AppUtils.getWindowInfo(context);
        Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(),
                imgId);
        bitmap = Bitmap.createScaledBitmap(bitmap, (int) (20* metric.density), (int) (20* metric.density), true);
    ...
    public int getGifName(Context context, String key)
    {
        String value = emojiMapGif.get(key);
        int resId = 0;
         resId = context.getResources().getIdentifier(value, "drawable",
                context.getPackageName());
        return resId;
    }

    public int getPngName(Context context, String key)
    {
        String value = emojiMap.get(key);
        int resId = 0;
        resId = context.getResources().getIdentifier(value, "drawable",
    ...
    public void getFileText(Context context) {
        ParseData(getEmojiFilePng(context), context);
    }

    public void getFileText_g(Context context) {
        ParseData_g(getEmojiFileGif(context), context);
    }
    ...
    private void ParseData_g(List<String> data, Context context) {
        if (data == null) {
            return;
        }
        ChatEmoji emojEentry;
        try {
            for (String str : data) {
                ...
                String fileName = text[0]
                        .substring(0, text[0].lastIndexOf("."));
                emojiMapGif.put(text[1], fileName);
                int resID = context.getResources().getIdentifier(fileName,
                        "drawable", context.getPackageName());
            ...
            int pageCount = (int) Math.ceil(emojisg.size() / 20 + 0.1);

            for (int i = 0; i < pageCount; i++) {
                emojiListsg.add(getData(i));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private List<ChatEmoji> getData(int page) {
        ...
        if (list.size() == pageSize) {
            ChatEmoji object = new ChatEmoji();
            object.setId(R.drawable.im_face_del_icon);
            list.add(object);
        }
        return list;
    }
