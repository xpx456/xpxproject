    ...
    public static void praseSign(NetObject net, Context context, SignAdapter adapter) {
        try {
            ...
            for (int i = 0; i < array.length() - 1; i++) {
                ...
                String[] addresss = item.getString("address").split(",");
                String ad = "";
                if(addresss != null)
                {
                ...
                adapter.mAttendanceModels.add(new Sign(item.getString("signin_id"),
                        SignManager.getInstance().oaUtils.mContactManager.mOrganization.getContactName(item.getString("user_id"),context),
                        point.getLatitude(), point.getLongitude()
                        , item.getString("address"), dete, time, item.getString("address"), item.getString("reason"),
                        item.getString("remark"),item.getString("image"),ad));
