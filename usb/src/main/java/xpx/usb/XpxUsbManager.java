...
public class XpxUsbManager {
    ...
    public byte [] start = {0x25, 0x03, 0x12, 0x01, (byte) 0xa0};
    public byte [] stop = {0x25, 0x09, 0x00, 0x01, (byte) 0xa0};
    public byte [] check = {0x25, 0x08, 0x00, 0x01, (byte) 0xa0};
    ...
    public static XpxUsbManager init2(Context context) {
    ...
    public void getOrgData(byte[] data)
    {
        if(data.length > 0){
            if(getOrgData != null)
            {
                getOrgData.receiveData(data);
            }
        }
    }
            ...
            if(data.length >= 9)
            {
                if(data[8] == 0x3f)
                {
                    if(data[1] == 0x15 || data[1] == 0x25) {
                        ...
                        if((System.currentTimeMillis() - changecurrent) > XpxUsbManager.CHANGE_TIME && change == true)
                        {
                            dataget[5] = "1";
