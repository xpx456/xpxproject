...
public class ImPrase {
                ...
                for (int i = 0;i<ja.length();i++) {
                    ...
                    if (mConversation.sourceType != Conversation.MESSAGE_TYPE_NOMAL) {
                        ...
                        if (mConversation.sourceType == Conversation.MESSAGE_TYPE_FILE) {
                            mConversation.mSubject = context.getString(R.string.file);
                            mConversation.sourcePath = InterskyApplication.mApp.mFileUtils.pathUtils.getfilePath("/im/file/"
                            ...
                            mConversation.mSubject = context.getString(R.string.pic);
                            mConversation.sourcePath = InterskyApplication.mApp.mFileUtils.pathUtils.getfilePath("/im/img/"
                                    +mConversation.detialId)+"/"+mConversation.sourceName;
                            file.add(mConversation);
                            Conversation source = new Conversation(mConversation);
                            Message message = new Message();
                            ...
                            mConversation.mSubject = context.getString(R.string.audio);
                            mConversation.sourcePath = InterskyApplication.mApp.mFileUtils.pathUtils.getfilePath("/im/voice/"
                                    +mConversation.detialId)+"/"+mConversation.sourceName;
                            Conversation source = new Conversation(mConversation);
                            Message message = new Message();
                            message.obj = new Conversation(source);
                            ...
                            mConversation.mSubject = context.getString(R.string.location)+":"+content.getString("locationName");
                            mConversation.sourceName = map.toString();
                            file.add(mConversation);
                            mConversation.sourcePath = InterskyApplication.mApp.mFileUtils.pathUtils.getfilePath("/im/map/"
                                    +mConversation.detialId+"/"+mConversation.sourceName);
                        }
                    }
    ...
    public static void praseConversationList(String json,Context context) {
        try {
            XpxJSONObject xpxJsonObject = new XpxJSONObject(json);
            XpxJSONArray ja = xpxJsonObject.getJSONArray("data");
            for (int i = 0; i < ja.length() - 1; i++) {
                XpxJSONObject jo = ja.getJSONObject(i);
                praseConversation(jo.toString(),context);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public static void praseConversation(String json,Context context) {
        try {
            ...
            info.mTitle = ContactManager.mContactManager.mOrganization.getContactName(jo.getString("source_id"),context) + jo.getString("title");
