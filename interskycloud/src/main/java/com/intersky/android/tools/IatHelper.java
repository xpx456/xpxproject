...
//                showTip("initfail，code：" + code);
                AppUtils.showMessage(mContext,"initfail，code：" + code);
            }
        }
    };
...
//            }
    ...
    private RecognizerListener mRecognizerListener = new RecognizerListener() {
        ...
        public void onError(SpeechError error) {
            // Tips：
            // code：10118(您没有说话)，可能是录音机权限被禁，需要提示用户打开应用的录音权限。
        }

        @Override
        public void onEndOfSpeech() {
            // 此回调表示：检测到了语音的尾端点，已经进入识别过程，不再接受语音输入
            AppUtils.showMessage(mContext,"speak finish");
        }
