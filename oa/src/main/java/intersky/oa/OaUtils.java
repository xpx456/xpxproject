...
public class OaUtils {
    ...
    public void uploadAttchments2(File file, Handler handler) {
        ...
        if(items.size() > 0)
        {
            OaDataItem oaDataItem = new OaDataItem(handler,items,"");
            oaDataItem.type = 1;
            Message message = new Message();
            message.what = OaHandler.EVENT_UPLOAD_FILES;
            message.obj = oaDataItem;
    ...
    public void doSendFiles(OaDataItem mOaDataItem) {
        String urlString = "";
        if(mOaDataItem.type == 0)
        {
            urlString = createURLStringoa(service,OaAsks.ATTACHMENT_PATH);
            NameValuePair nameValuePair = new NameValuePair("method",OaAsks.ATTACHMENT_LIST);
            mOaDataItem.nameValuePairs.add(nameValuePair);
        }
        else
             urlString = NetUtils.getInstance().praseUrl(OaUtils.getOaUtils().service,OaAsks.ATTACHMENT_PATH2);
