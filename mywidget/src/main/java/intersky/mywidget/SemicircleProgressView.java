...
//        matrix.postTranslate(pos[0] - bitmap.getWidth() / 2, pos[1] - bitmap.getHeight() / 2);
    ...
    private void drawCenterText(Canvas canvas) {
        ...
        canvas.drawText(Title, (maxWidth / 2), (maxHeight / 2) - dp2px(0), mTextPaint);
        //绘制SubTile
        mTextPaint2.setTextSize(semicirclesubtitleSize);
        canvas.drawText(SubTile, (maxWidth / 2), (maxHeight / 2) + dp2px(25), mTextPaint2);
    ...
    public void setSesameValues(int values, int total) {
        if (values >= 0) {
            ...
            Title = String.valueOf(values);
                ...
                Title = String.valueOf(values)+"+";
            sesameLevel = String.valueOf(total);
            mTotalAngle = ((float) values / (float) total) * 290f;
            startAnim();
        }
    }
