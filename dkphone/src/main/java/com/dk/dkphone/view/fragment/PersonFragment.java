        ...
        public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
            ...
            if (DkPhoneApplication.mApp.pk != null) {
                if (user.select == true) {
                    ...
                    DkPhoneApplication.mApp.pk.select = false;
                    DkPhoneApplication.mApp.pk = user;
                    DkPhoneApplication.mApp.pk.select = true;
                    showCardView();
                }
...
//        initView();
    ...
    public void setMyView() {
        if (DkPhoneApplication.mApp.selectUser != null) {
            if (DkPhoneApplication.mApp.selectUser.headpath.length() > 0) {
                File file = new File(DkPhoneApplication.mApp.selectUser.headpath);
                if (file.exists()) {
                    Glide.with(mMainActivity).load(new File(DkPhoneApplication.mApp.selectUser.headpath)).into(new MySimpleTarget(myHead));
                }
            }
            else
            {
