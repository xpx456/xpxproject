...
public class SettingPresenter implements Presenter {
    ...
    private void updataView() {
        ...
        mSettingActivity.nowVersion.setText(DkPadApplication.mApp.versionName);
        if(DkPadApplication.mApp.mUpDataManager.updataVersionCode > DkPadApplication.mApp.mUpDataManager.oldVersionCode
        && DkPadApplication.mApp.mUpDataManager.finish == true)
        {
            mSettingActivity.updataVersion.setText(DkPadApplication.mApp.mUpDataManager.updataVersionName);
...
//            imm2.hideSoftInputFromWindow(name.getWindowToken(), 0);
    ...
    private void doUpdata() {
        File file = new File(DkPadApplication.mApp.mUpDataManager.path);
        if(file.exists() )
            mSettingActivity.startActivity(DkPadApplication.mApp.fileUtils.openfile(
                    new File(DkPadApplication.mApp.mUpDataManager.path)));
        else
    ...
    private void setMax(String pass) {
        if(Integer.valueOf(pass) <= 32)
        {
            ...
            DkPadApplication.mApp.maxSelect = Integer.valueOf(pass);
    ...
    {
        DkPadApplication.mApp.audioManager.audioSetting(mSettingActivity);
    }
    ...
    private void setPassword2(String pass,PopupWindow popupWindow) {
        ...
        if(passd.equals(pass) ||(pass.length() == 0 && passd.length() == 0))
        {
            ...
            DkPadApplication.mApp.audioManager.speak(mSettingActivity,mSettingActivity.getString(R.string.setting_passord_error));
        }
    }
