    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case TaskAsks.TASK_LIST_SUCCESS:
                ...
                if(mTaskFragment != null)
                {
                    if (mTaskFragment.mSearchViewLayout.ishow == true) {
                        if(TaskManager.getInstance().taskOldSearchPage > TaskManager.getInstance().taskSearchPage && TaskManager.getInstance().taskSearchAll == false)
                        {
                        ...
                        else
                        {
                            ...
                            if(theActivity.mTaskSearchAdapter != null)
                            {
                                theActivity.mTaskAdapter.notifyDataSetChanged();
                            }
                            theActivity.waitDialog.hide();
            ...
            case ProjectFileAsks.PROJECT_MOVE_OUT_SUCCESS:
                theActivity.waitDialog.hide();
                Project item1 = ProjectFilePrase.praseMove((NetObject) msg.obj);
                if(item1 != null)
                    theActivity.mTaskManagerPresenter.initMove(item1);
                break;
