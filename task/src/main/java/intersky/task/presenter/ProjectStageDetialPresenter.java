...
public class ProjectStageDetialPresenter implements Presenter {
    ...
    public void initView() {
        ...
        ToolBarHelper.setRightBtn(mProjectStageDetialActivity.mActionBar, mProjectStageDetialActivity.newTask, mProjectStageDetialActivity.showDetial, R.drawable.headcreat, R.drawable.p_info);
    }
...
//                title.setTextColor(Color.rgb(0, 0, 0));
            ...
            if (task.endTime.length() > 0) {
                    ...
                    if (task.isComplete == 1) {
                        ...
                        if(task.beginTime.length() > 0)
                        {
                            if (TimeUtils.minuteBetween(TimeUtils.getDateAndTime(), task.beginTime) > 0) {
                                dete.setText(TimeUtils.praseTaskItemdata(task.beginTime) + "~" + TimeUtils.praseTaskItemdata(task.endTime));
                        ...
                        {
                            dete.setText(TimeUtils.praseTaskItemdata(task.endTime)+mProjectStageDetialActivity.getString(R.string.task_date_end));
                            dete.setTextColor(Color.rgb(118, 118, 118));
                        }
                ...
                if (task.beginTime.length() > 0 && task.isComplete == 0) {
                    dete.setVisibility(View.VISIBLE);
                    if (TimeUtils.minuteBetween(TimeUtils.getDateAndTime(), task.beginTime) > 0) {
                        dete.setText(TimeUtils.praseTaskItemdata(task.beginTime)  +mProjectStageDetialActivity.getString(R.string.task_date_start));
                        dete.setTextColor(Color.rgb(118, 118, 118));
                    }
                    else
                    {
