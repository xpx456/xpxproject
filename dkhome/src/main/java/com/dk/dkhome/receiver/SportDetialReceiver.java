...
public class SportDetialReceiver extends BaseReceiver {


	private Handler mHandler;

	public SportDetialReceiver(Handler mHandler)
	{
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(intent.getAction().equals(DeviceView.ACTION_UPDTAT_DEIVECONNECT_IMF)){
			...
			Message message = new Message();
			message.obj = intent;
			message.what = SportDetialHandler.UPDTATA_DEVICE_STATE;
			if(mHandler != null)
			{
				mHandler.sendMessage(message);
			}
			...
			Message message = new Message();
			message.obj = intent;
			message.what = SportDetialHandler.SET_SPORT_TYPE;
			if(mHandler != null)
			{
				mHandler.sendMessage(message);
			}
			...
			Message message = new Message();
			message.obj = intent;
			message.what = SportDetialHandler.SET_SPORT_TYPE_NAME;
			if(mHandler != null)
			{
				mHandler.sendMessage(message);
			}
			...
			Message message = new Message();
			message.obj = intent;
			message.what = SportDetialHandler.UPDATA_DEVICE_LIST;
			if(mHandler != null)
			{
				mHandler.sendMessage(message);
			}
			...
			Message message = new Message();
			message.obj = intent;
			message.what = SportDetialHandler.CONNECT_FINISH;
			if(mHandler != null)
			{
				mHandler.sendMessage(message);
			}
		...
		else if(intent.getAction().equals(ScanUtils.ACTION_SCAN_FINISH))
		{
			...
			msg.what = SportDetialHandler.SCAN_FINISH;
			msg.obj = intent;
			if(mHandler!=null)
