...
public class OaMessagePrase {
        ...
        try {
            ...
            if (jo.getInt("total_results",0) > mainActivity.unreadcount) {
                ...
                ConversationAsks.getOaMessages(ConversationManager.getInstance().context, mainActivity.messagePage, mainActivity.mMainPresenter.mMainHandler
                        , StrangApplication.mApp.mAccount.mRecordId,StrangApplication.mApp.mAccount.mCompanyId);
            } else {
                if (mainActivity.readid.length() > 0)
                    ConversationAsks.setRead(ConversationManager.getInstance().context, mainActivity.mMainPresenter.mMainHandler,
                            mainActivity.readid,StrangApplication.mApp.mAccount.mRecordId,StrangApplication.mApp.mAccount.mCompanyId);
    ...
    public static void praseGropMessage(Context context,NetObject net) {
        ...
        try {
            ...
            for (int i = 0; i < ja.length() - 1; i++) {
                ...
                StrangApplication.mApp.conversationManager.doAdd(tConversationModel);
                if(start != null) {
    ...
    public static void praseConversation(String json) {
        try {
            ...
            {
                if(StrangApplication.mApp.contactManager.mOrganization.mAllContactsMap.containsKey(jo.getString("source_id")))
                {
                    info.mTitle = StrangApplication.mApp.contactManager.mOrganization.mAllContactsMap.get(jo.getString("source_id")).getmRName() + jo.getString("title");
                }
                else
                {
                    info.mTitle = StrangApplication.mApp.getString(R.string.keyword_unknow);
                }
            }
            ...
            StrangApplication.mApp.conversationManager.doAdd(info);
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
