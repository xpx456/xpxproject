...
import com.intersky.strang.android.receiver.MainReceiver;
import com.intersky.strang.android.view.StrangApplication;
import com.intersky.strang.android.view.activity.AboutActivity;
...
import com.intersky.strang.android.view.fragment.ContactsFragment;
import com.intersky.strang.android.view.fragment.ConversationFragment;
import com.intersky.strang.android.view.fragment.MyFragment;
import com.intersky.strang.android.view.fragment.WorkFragment;
...
public class MainPresenter implements Presenter {
    ...
    public void initView() {
        ...
        mMainActivity.iatHelper = new IatHelper(mMainActivity,StrangApplication.mApp.commendManager);
        mMainActivity.iatHelper.initArs();
        StrangApplication.mApp.conversationManager.registerConversations(StrangApplication.mApp.registers, mMainActivity.baseReceiver, mMainActivity);
        StrangApplication.mApp.notifictionManager = NotifictionManager.init(StrangApplication.mApp, R.mipmap.small_icon,R.mipmap.icon,
                StrangApplication.mApp.conversationManager.channels,
                "com.intersky.android.receiver.NotifictionOpenReceiver",StrangApplication.mApp.dataMeasure);
        StrangApplication.mApp.downloadOper = StrangApplication.mApp.notifictionManager.
                getOper(ConversationManager.getInstance().getChannel(IntersakyData.CONVERSATION_TYPE_DOCUMENT));
        StrangApplication.mApp.updataOper = StrangApplication.mApp.notifictionManager.
        ...
        StrangApplication.mApp.conversationManager.getDataBaseData();
        ...
        mContactAdapter = new ContactsAdapter(StrangApplication.mApp.contactManager.mOrganization.allClassContacts, mMainActivity);
...
//                String[] ss = new String[StrangApplication.mApp.contactManager.mOrganization.allContactsHead.size()];
//                for (int i = 0; i < StrangApplication.mApp.contactManager.mOrganization.allContactsHead.size(); i++) {
//                    ss[i] = StrangApplication.mApp.contactManager.mOrganization.allContactsHead.get(i).getName();
//                }
    ...
    public void showMore() {
        ...
        animation.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationStart(Animation animation) {
                ...
                mGrid.setAdapter(new FunctionAdapter(mMainActivity, StrangApplication.mApp.functionUtils.moreFunctions, true));
                mGrid.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    @Override
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        StrangApplication.mApp.functionUtils.startFunction(mMainActivity,(Function)parent.getAdapter().getItem(position),StrangApplication.mApp.mAccount.iscloud);
                        doclose();
                    }
                });
...
//        StrangApplication.mApp.conversationManager.getLeaveOaMessage(mMainHandler);
    }
    ...
    public void LetterChange(int s) {
        Contacts model = StrangApplication.mApp.contactManager.mOrganization.allContactsHead.get(s);
        int a =StrangApplication.mApp.contactManager.mOrganization.allContacts.indexOf(model);
    ...
    public void contactsClick(Contacts contacts) {
        ...
        {
            StrangApplication.mApp.contactManager.startContactDetial(mMainActivity,contacts);
        }
        else if(contacts.mType == Contacts.TYPE_CLASS)
        {
            StrangApplication.mApp.contactManager.startContactList(mMainActivity,contacts);
        }
    }
    ...
    {
        FunctionUtils.getInstance().startFunction(mMainActivity,mFunction,StrangApplication.mApp.mAccount.iscloud);
    }

    public void openConversationActivity(Intent intent) {
        String type = intent.getStringExtra("type");
        Register register = StrangApplication.mApp.conversationManager.conversationAll.getRegister(type);
        if(register != null) {
...
//        intent.putExtra("conversation", mConversationModel);
    ...
    public void doClean() {
        File mfile = new File(StrangApplication.mApp.mFileUtils.pathUtils.getfilePath("/mail"));
        if (mfile.exists()) {
            StrangApplication.mApp.mFileUtils.pathUtils.recursionDeleteFile(mfile);
            mfile.delete();
        }
        mfile.mkdirs();
        mfile = new File(StrangApplication.mApp.mFileUtils.pathUtils.getfilePath("/task"));
        if (mfile.exists()) {
            StrangApplication.mApp.mFileUtils.pathUtils.recursionDeleteFile(mfile);
            mfile.delete();
        }
        mfile.mkdirs();
        mfile = new File(StrangApplication.mApp.mFileUtils.pathUtils.getfilePath("/grid"));
        if (mfile.exists()) {
            StrangApplication.mApp.mFileUtils.pathUtils.recursionDeleteFile(mfile);
            mfile.delete();
        }
        mfile.mkdirs();
        mfile = new File(StrangApplication.mApp.mFileUtils.pathUtils.getfilePath("/download"));
        if (mfile.exists()) {
            StrangApplication.mApp.mFileUtils.pathUtils.recursionDeleteFile(mfile);
            mfile.delete();
        }
        mfile.mkdirs();
        mfile = new File(StrangApplication.mApp.mFileUtils.pathUtils.getfilePath("/warm"));
        if (mfile.exists()) {
            StrangApplication.mApp.mFileUtils.pathUtils.recursionDeleteFile(mfile);
            mfile.delete();
        }
        mfile.mkdirs();
        mfile = new File(StrangApplication.mApp.mFileUtils.pathUtils.getfilePath("/attdence"));
        if (mfile.exists()) {
            StrangApplication.mApp.mFileUtils.pathUtils.recursionDeleteFile(mfile);
            mfile.delete();
        }
        mfile.mkdirs();
        mfile = new File(StrangApplication.mApp.mFileUtils.pathUtils.getfilePath("/cicle"));
        if (mfile.exists()) {
            StrangApplication.mApp.mFileUtils.pathUtils.recursionDeleteFile(mfile);
            mfile.delete();
        }
        mfile.mkdirs();
        mfile = new File(StrangApplication.mApp.mFileUtils.pathUtils.getfilePath("/report"));
        if (mfile.exists()) {
            StrangApplication.mApp.mFileUtils.pathUtils.recursionDeleteFile(mfile);
            mfile.delete();
        }
        mfile.mkdirs();
        mfile = new File(StrangApplication.mApp.mFileUtils.pathUtils.getfilePath("/photo"));
        if (mfile.exists()) {
            StrangApplication.mApp.mFileUtils.pathUtils.recursionDeleteFile(mfile);
            mfile.delete();
        }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            ...
            {
                StrangApplication.mApp.existApplacation();
                StrangApplication.mApp.appActivityManager.AppExit(mMainActivity);
            }
            else
            {
