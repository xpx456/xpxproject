...
public class FingerManger {
    ...
    {
        int a = fingerManger.jxVeinJavaAPI.jxIsVeinDevConnected();
        if(fingerManger.jxVeinJavaAPI.jxIsVeinDevConnected() <= 0)
        {
            disconnect = true;
        ...
        if(fingerManger.fingerHandler != null)
            fingerManger.fingerHandler.sendEmptyMessageDelayed(FingerHandler.CHECK_CONNECT,1000);

    }

    public void getPermissionOnly()
    {
...
//    private void getUsbPermission2(UsbDevice mUSBDevice) {
//        PendingIntent pendingIntent = PendingIntent.getBroadcast(context, 0, new Intent(ACTION_USB_PERMISSION), 0);
