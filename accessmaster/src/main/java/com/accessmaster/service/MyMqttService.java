package com.accessmaster.service;
...
import com.accessmaster.receiver.MqttReceiver;
...
//        init(intent.getStringExtra("clientid"),intent.getStringExtra("host"));
    ...
    public void init(String host,String user) {
        ...
        if(reconnect == true)
        {
            ...
            mMqttConnectOptions.setUserName("dashboard");
    ...
    private void doClientConnection() {
        if (!mqttAndroidClient.isConnected() && isConnectIsNomarl()) {
            try {
                mqttAndroidClient.connect(mMqttConnectOptions, AccessMasterApplication.mApp, iMqttActionListener);
            } catch (MqttException e) {
                e.printStackTrace();
            }
        }
    ...
    private IMqttActionListener iMqttActionListener = new IMqttActionListener() {
        ...
        public void onSuccess(IMqttToken arg0) {
            ...
            try {
                ...
                mqttAndroidClient.subscribe(MqttManager.TOPIC_HOME, 2);
                mqttAndroidClient.subscribe(CLIENTID+MqttManager.TOPIC_HOME, 2);
                Intent intent = new Intent(ACTION_SERVICE_CONNECTED);
                if(AccessMasterApplication.mApp != null)
    ...
    private MqttCallback mqttCallback = new MqttCallback() {
        ...
        public void messageArrived(String topic, MqttMessage message) throws Exception {
            ...
            if(topic.equals(MqttManager.TOPIC_HOME))
            {
                String[] back = MqttPrase.praseGuest2(json);
                //if(back[0].length() > 0)
                //response(back[0],back[1]+MqttManager.TOPIC_GUEST);
            }
            else if(topic.equals(AccessMasterApplication.mApp.clidenid+MqttManager.TOPIC_HOME))
            {
                String[] back = MqttPrase.praseGuest2(json);
                //if(back[0].length() > 0)
                //response(back[0],back[1]+MqttManager.TOPIC_GUEST);
