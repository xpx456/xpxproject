...
//    };
    ...
    public View.OnClickListener regiserListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            {
                String ip = "";
                if(AccessMasterApplication.newPad == false)
                 ip = AccessMasterApplication.mApp.manager.ZYgetEthIp();
                else
                  ip = AppUtils.getLocalIpAddress2(mRegisterActivity);
                ...
                DeviceAsks.registerDevice(mRegisterActivity,registerHandler,device,ip);
                    ...
                    AppUtils.showMessage(mRegisterActivity,"请先配置服务器");
            }
            else if(AppUtils.getNetWorkState(mRegisterActivity) == 1)
            {
                String ip = "";
                if(AccessMasterApplication.newPad == false)
                    ip = AccessMasterApplication.mApp.manager.ZYgetWifiIp();
                else
                    ip = AppUtils.getLocalIpAddress2(mRegisterActivity);
                if(AccessMasterApplication.mApp.appservice.sAddress.length() > 0 && AccessMasterApplication.mApp.appservice.sPort.length() > 0)
                DeviceAsks.registerDevice(mRegisterActivity,registerHandler,device,ip);
