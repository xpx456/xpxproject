...
public class MainPresenter implements Presenter {
    ...
    public void initView() {
        ...
        mainHandler.sendEmptyMessageDelayed(MainHandler.UPDATA_DEVICES,120*1000);
    }
    ...
    public void updataGetdevices() {
        if(AccessMasterApplication.mApp.myMqttService.mqttAndroidClient.isConnected())
        MqttAsks.getDevices(mMainActivity,AccessMasterApplication.mApp.aPublic);
        if(mainHandler != null)
            mainHandler.sendEmptyMessageDelayed(MainHandler.UPDATA_DEVICES,120*1000);
    }
    ...
    public AdapterView.OnItemClickListener onItemClickListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
            if (ActivityCompat.checkSelfPermission(mMainActivity, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED)
            {
                ...
                if(AccessMasterApplication.mApp.chatresource == false && AccessMasterApplication.mApp.isshow == false)
                {
                    ...
                    {
                        if(NetUtils.checkNetWorkState(mMainActivity))
                        MqttAsks.showView(mMainActivity,AccessMasterApplication.mApp.aPublic,device.cid);
                        else
                            AppUtils.showMessage(mMainActivity,"没有网络连接");
                    }
