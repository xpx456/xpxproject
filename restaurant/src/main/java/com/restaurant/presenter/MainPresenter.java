...
//    }
    ...
    public void creatSuccess(String[] data,Guest guest) {
        ...
        accessRecord.accessRecordTime = TimeUtils.getDateAndTime();
        accessRecord.authModeCode = guest.mode;
        accessRecord.licence = guest.licence;
        accessRecord.name = guest.name;
        accessRecord.entEqptNo = RestaurantApplication.mApp.clidenid;
        if(DBHelper.getInstance(mMainActivity).addRecord(accessRecord) > 0)
        {
            MqttAsks.uploadRecord(mMainActivity,accessRecord,RestaurantApplication.mApp.aPublic);
        }
...
//                AppUtils.showMessage(mMainActivity,"识别为"+guest.name);
                    ...
                    {
                        RestaurantApplication.mApp.soundManager.doPlayOneTimeSound(R.raw.fail);
    ...
    public void senduplod(String id,String mode) {
        if(mMainActivity.waitDialog.isshow() == false)
        {
            ...
            guest.mode = mode;
//            AppUtils.showMessage(mMainActivity,"识别为"+guest.name);
            DinnerAsks.sendDinner(mMainActivity,mainHandler,id,guest);
        }
    }
    ...
    public void sendSuccess(NetObject netObject) {
        try {
            ...
            Guest guest = (Guest) netObject.item;
