...
//            mNewReportActivity.mworktime = (RelativeLayout) mNewReportActivity.findViewById(R.id.worktime);
            ...
            mNewReportActivity.worktimename.setOnClickListener(mNewReportActivity.detepickListener);
        ...
        ImageView back = mNewReportActivity.findViewById(R.id.back);
        ...
        mNewReportActivity.mImageLayer = (MyLinearLayout) mNewReportActivity.findViewById(R.id.image_content);

        TextView save = mNewReportActivity.findViewById(R.id.publish);
        save.setOnClickListener(mNewReportActivity.saveListener);
    ...
    private void initAddTextView() {
        ...
        mTextView.setOnClickListener(mNewReportActivity.mShowAddListener);
        mNewReportActivity.mImageLayer.addView(mView);
    ...
    public void removePic(View view) {
        ...
        mNewReportActivity.mImageLayer.removeView(v);
        int size = mNewReportActivity.mPics.size();
        mNewReportActivity.mPics.remove(attachment);
        LayoutInflater mInflater = (LayoutInflater) mNewReportActivity.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        if(mNewReportActivity.mPics.size() < WorkReportManager.MAX_PIC_COUNT && size == WorkReportManager.MAX_PIC_COUNT)
        {
            View mView = mInflater.inflate(R.layout.fujian_item_add_up, null);
            ImageView mTextView = (ImageView) mView.findViewById(R.id.add_title);
            mTextView.setOnClickListener(mNewReportActivity.mShowAddListener);
...
//        mNewReportActivity.mImageLayer.removeAllViews();
    ...
    public void praseAttahcmentViews() {
        ...
        for(int i = 0 ; i < mNewReportActivity.mPics.size() ; i++)
        {
            ...
            Bus.callData(mNewReportActivity,"filetools/addPicView3",attachment,url,mNewReportActivity.mImageLayer,mNewReportActivity.mDeletePicListener);
        }
    }
    ...
    public void setpic(Intent intent) {
        ...
        if(attachments.size()+mNewReportActivity.mPics.size() > WorkReportManager.MAX_PIC_COUNT)
        {
    ...
    public void initPicView(ArrayList<Attachment> mPics) {
        for (int i = 0; i < mPics.size(); i++) {
            Bus.callData(mNewReportActivity,"filetools/addPicView3",mPics.get(i),"",mNewReportActivity.mImageLayer,mNewReportActivity.mDeletePicListener);
        }
        if (mNewReportActivity.mPics.size() == WorkReportManager.MAX_PIC_COUNT) {
            View mview = mNewReportActivity.mImageLayer.getChildAt(mNewReportActivity.mImageLayer.getChildCount() - 1);
            mNewReportActivity.mImageLayer.removeView(mview);
        }
    }

    public void selectSender() {
        Bus.callData(mNewReportActivity,"chat/selectListContact",mNewReportActivity.mSenders, WorkReportManager.ACTION_REPORT_UPATE_SENDER, mNewReportActivity.getString(R.string.select_contact), false);
    }

    public void selectCopyer() {
        Bus.callData(mNewReportActivity,"chat/selectListContact",mNewReportActivity.mCopyers, WorkReportManager.ACTION_REPORT_UPATE_COPYER,  mNewReportActivity.getString(R.string.select_contact), false);
    }
    ...
    {
        if(mNewReportActivity.popupWindow1 != null)
        {
            mNewReportActivity.popupWindow1.dismiss();
        }
    }
        ...
        switch (requestCode) {
            case Actions.TAKE_PHOTO:
                ...
                if (mFile.exists()) {
                    ...
                    Bus.callData(mNewReportActivity,"filetools/addPicView3",attachment,"",mNewReportActivity.mImageLayer,mNewReportActivity.mDeletePicListener);
                    if (mNewReportActivity.mPics.size() == WorkReportManager.MAX_PIC_COUNT) {
                        View mview = mNewReportActivity.mImageLayer.getChildAt(mNewReportActivity.mImageLayer.getChildCount() - 1);
                        mNewReportActivity.mImageLayer.removeView(mview);
                    }
    ...
    public void showAdd() {
        ...
        menuItem.btnName = mNewReportActivity.getString(R.string.button_word_takephoto);
        menuItem.mListener = mNewReportActivity.mTakePhotoListenter;
        items.add(menuItem);
        menuItem = new MenuItem();
        menuItem.btnName = mNewReportActivity.getString(R.string.button_word_album);
        menuItem.mListener = mNewReportActivity.mAddPicListener;
        items.add(menuItem);
        mNewReportActivity.popupWindow1 = AppUtils.creatButtomMenu(mNewReportActivity,mNewReportActivity.mshada,items,mNewReportActivity.findViewById(R.id.activity_about));
    }

    public void askFinish()
    {
        AppUtils.creatDialogTowButton(mNewReportActivity, mNewReportActivity.getString(R.string.save_ask),
                mNewReportActivity.getString(R.string.save_ask_title),mNewReportActivity.getString(R.string.button_word_cancle)
                ,mNewReportActivity.getString(R.string.button_word_ok),null,new DialogInterface.OnClickListener(){

                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        mNewReportActivity.finish();
                    }
                });
    }
    ...
    public void chekcBack() {
        ...
        if(mNewReportActivity.mReport.mRecordid.length() > 0)
        {
            if (mNewReportActivity.mRemark.getText().toString().replaceAll(" ", "").length() > 0) {
        ...
        {
            if (mNewReportActivity.mRemark1.getText().toString().replaceAll(" ", "").length() > 0) {
        ...
        if(mNewReportActivity.mReport.mRecordid.length() > 0)
        {
            if (mNewReportActivity.mWorkHelp.getText().toString().replaceAll(" ", "").length() > 0) {
        ...
        {
            if (mNewReportActivity.mWorkHelp1.getText().toString().replaceAll(" ", "").length() > 0) {
        ...
        if (mNewReportActivity.mPics.size() > 0) {
            askFinish();
            return;
        }
        mNewReportActivity.finish();
    ...
    {
        if (e1.getX() - e2.getX() > mNewReportActivity.mBasePresenter.mScreenDefine.verticalMinDistance*mNewReportActivity.mBasePresenter.mScreenDefine.density && Math.abs(velocityX) > 0)
        {
            return false;
        }
        else if (e2.getX() - e1.getX() > mNewReportActivity.mBasePresenter.mScreenDefine.verticalMinDistance*mNewReportActivity.mBasePresenter.mScreenDefine.density && Math.abs(velocityX) > 0)
        {

            if (e2.getX() - e1.getX() > e2.getY() - e1.getY() && e2.getX() - e1.getX() > e1.getY() - e2.getY() && mNewReportActivity.flagFillBack == true)
            {
                mNewReportActivity.isdestory = true;
                chekcBack();
                return true;
            }
            else
