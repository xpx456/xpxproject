...
//				.getAbsolutePath(), this);
        RelativeLayout mLinearLayout = (RelativeLayout) mBusinesCardActivity
                .findViewById(R.id.activity_busines2);
        mLinearLayout.setOnTouchListener(mBusinesCardActivity);
        mLinearLayout.setLongClickable(true);
        mBusinesCardActivity.mGestureDetector = new GestureDetector((OnGestureListener) mBusinesCardActivity);
        ToolBarHelper.setTitle(mBusinesCardActivity.mActionBar, mBusinesCardActivity.getString(R.string.keyword_businesscard));
        ToolBarHelper.setBackListenr(mBusinesCardActivity.mActionBar, mBusinesCardActivity.mBackListener);
        ToolBarHelper.setRightBtnText(mBusinesCardActivity.mActionBar, mBusinesCardActivity.mUpLoadBusines, mBusinesCardActivity.getString(R.string.keyword_output));
        mBusinesCardActivity.mListView = (ListView) mBusinesCardActivity.findViewById(R.id.busines_List);
        mBusinesCardActivity.msearchView = (EditText) mBusinesCardActivity.findViewById(R.id.sText);
        mBusinesCardActivity.msearchView.setText("");
        initData();
        mBusinesCardActivity.mListView.setOnItemClickListener(mBusinesCardActivity.mOnItemClickListener);
        mBusinesCardActivity.msearchView.addTextChangedListener(mBusinesCardActivity.mSearchWordChange);
    }
    ...
    public void initlistdata() {
        ...
        if (mBusinesCardActivity.msearchView.getText().toString().length() != 0) {
            for (int i = 0; i < mBusinesCardActivity.mBusinesItems.size(); i++) {
                if (mBusinesCardActivity.mBusinesItems.get(i).name.contains(mBusinesCardActivity.msearchView.getText().toString().toLowerCase()) || mBusinesCardActivity.mBusinesItems.get(i).getPingyin().contains(mBusinesCardActivity.msearchView.getText().toString().toLowerCase())) {
    ...
    public void upLoadBusines() {
        ...
        if (maddMkxCards.size() > 0) {
            ...
            AppUtils.showMessage(mBusinesCardActivity, "请选择导入名片");
        }
    }
