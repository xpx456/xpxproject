...
public class MailEditPresenter implements Presenter {
    ...
    public ArrayList<Select> lables = new ArrayList<Select>();
    ...
    public void initView() {
        ...
        ImageView back = mMailEditActivity.findViewById(R.id.back);
        back.setOnClickListener(mMailEditActivity.mBackListener);
        ...
        mMailEditActivity.mTab = (ImageView) mMailEditActivity.findViewById(R.id.tab_img);
        ...
        mMailEditActivity.mTabLayer = (RelativeLayout) mMailEditActivity.findViewById(R.id.tab_img_layer);
        ...
        mMailEditActivity.mTab.setOnClickListener(mTabListener);
        ...
        mMailEditActivity.mMailAllSelectAdapter = new MailSelectAdapter(mMailEditActivity, MailManager.getInstance().allSendMailBoxs);
        ...
        TextView add = mMailEditActivity.findViewById(R.id.send);
        title = mMailEditActivity.findViewById(R.id.title);
        imgtitle = mMailEditActivity.findViewById(R.id.titlearray);
        btngrop = mMailEditActivity.findViewById(R.id.btngrop);
        btnnomal = mMailEditActivity.findViewById(R.id.btnnomal);
        imgnomal = mMailEditActivity.findViewById(R.id.nomalimg);
        imggrop = mMailEditActivity.findViewById(R.id.gropimg);
        selectlayer = mMailEditActivity.findViewById(R.id.mail_send_select);
        btnnomal.setOnClickListener(setNomalMailListener);
        btngrop.setOnClickListener(setGropMailListener);
        switch (mMailEditActivity.mAction) {
            case MailManager.ACTION_NEW:
                title.setText(mMailEditActivity.getString(R.string.mail_write_nomal));
                add.setText(mMailEditActivity.getString(R.string.button_word_send));
                add.setOnClickListener(mMailEditActivity.mSendListener);
                break;
            case MailManager.ACTION_REPEAT:
            case MailManager.ACTION_REPEATALL:
                title.setText(mMailEditActivity.getString(R.string.mail_repickmail));
                add.setText(mMailEditActivity.getString(R.string.button_word_send));
                add.setOnClickListener(mMailEditActivity.mSendListener);
                break;
            case MailManager.ACTION_EDIT:
                title.setText(mMailEditActivity.getString(R.string.mail_resendmail));
                add.setText(mMailEditActivity.getString(R.string.button_word_save));
                add.setOnClickListener(mMailEditActivity.mSendListener);
                break;
            case MailManager.ACTION_RESEND:
                title.setText(mMailEditActivity.getString(R.string.mail_editmail));
                add.setText(mMailEditActivity.getString(R.string.button_word_send));
                add.setOnClickListener(mMailEditActivity.mSendListener);
                break;
        }
        ...
        if (mMailEditActivity.mAction != MailManager.ACTION_NEW) {
            ...
            if (MailManager.getInstance().account.iscloud == false)
            {
                mMailEditActivity.mTabLayer.setVisibility(View.INVISIBLE);
                imgtitle.setVisibility(View.INVISIBLE);
            }
            else
            {
                imgtitle.setOnClickListener(selectSendTypeListener);
                title.setOnClickListener(selectSendTypeListener);
                imgtitle.setVisibility(View.VISIBLE);
                mMailEditActivity.mTabLayer.setVisibility(View.VISIBLE);
                initLables();
            }
        }
    ...
    public void praseMail() {
        if (mMailEditActivity.mMail != null) {
            ...
            initMailDetial();
            MailManager.getInstance().setMailBox(mMailEditActivity.mMail.mMailBoxId);
            //mMailEditActivity.mFajian.setText(mMailEditActivity.mMail.get);
        } else {
...
//				mMailEditActivity.waitDialog.setCancelable(false);
    ...
    public void onfajianlistener() {
        if(mMailEditActivity.sentquery)
        {
            showSelectDialog();
        }
        else
...
//			mMailEditActivity.mfujianLayer.setVisibility(View.VISIBLE);
    ...
    public void praseUploadNext(NetObject netObject) {
        ...
        if (json.length() == 0) {
            ...
            {
                if(MailManager.getInstance().nowAttachmentUploadThread.attachment.stata == Attachment.STATA_DOWNLOADFINISH ||
                        MailManager.getInstance().nowAttachmentUploadThread.attachment.stata == Attachment.STATA_DOWNLOADFIAL)
                {
                    if (MailManager.getInstance().upthreads.size() > 0) {
                        AttachmentUploadThread uploadThread = MailManager.getInstance().upthreads.get(0);
                        MailManager.getInstance().nowAttachmentUploadThread = uploadThread;
            ...
            try {
                ...
                if (jsonObject.getInt("code") == 0) {
                    ...
                    attachmentModel.mailUploadJson.put("size", praseAttachmentSize(data.getString("size")));
    ...
    public void getVideo() {
        Bus.callData(mMailEditActivity, "filetools/getVideos", false, "intersky.mail.view.activity.MailEditActivity", MailEditActivity.ACTION_MAIL_VIDEO_SELECT);
    }

    public void getPicture() {
        Bus.callData(mMailEditActivity, "filetools/getPhotos", false, 9, "intersky.mail.view.activity.MailEditActivity", MailEditActivity.ACTION_MAIL_PHOTO_SELECT);
    }
    ...
    public void selectMail(MailBox mMailBox) {
        ...
        if (mMailEditActivity.mAction == MailManager.ACTION_REPEAT) {
            if (mMailEditActivity.mMail.isLocal) {
                ...
                for (int i = 0; i < MailManager.getInstance().mailLContacts.size(); i++) {
                    if (MailManager.getInstance().mailLContacts.get(i).mRecordid.equals(mMailEditActivity.mMail.mCatalogueID)) {
    ...
    private void initMailPerosnView() {
        ...
        mTextView.setText(mMailEditActivity.getString(R.string.mail_title_shoujianren));
        ...
        mTextView1.setText(mMailEditActivity.getString(R.string.mail_title_neibuchaosong));
        ...
        mTextView2.setText(mMailEditActivity.getString(R.string.mail_title_chaosong));
        ...
        mTextView3.setText(mMailEditActivity.getString(R.string.mail_title_bcc));
    ...
    private void gethtmLMialPersons(ArrayList<MailContact> mMailPersonItems, String from) {
        ...
        for (int j = 0; j < str.length; j++) {
            for (int i = 0; i < MailManager.getInstance().mailLContacts.size(); i++) {
                if (str[j].equals(MailManager.getInstance().mailLContacts.get(i).mRecordid)) {
...
//		mMailEditActivity.mMail.setmContentHtml(mMailEditActivity.mMail.getmContentHtml() + "</BLOCKQUOTE>");
    ...
    private void showSelectDialog() {
        mMailEditActivity.selectDialog = MailEditPresenter.createSelectDialog(mMailEditActivity, mMailEditActivity.getString(R.string.mail_title_secect_sender));
        ListView mListView = (ListView) mMailEditActivity.selectDialog.findViewById(R.id.user_mail_List);
        if(mMailEditActivity.sentquery)
        {
            mListView.setAdapter(mMailEditActivity.mMailAllSelectAdapter);
        }
        else
        {
    ...
    private void sendMail(boolean issend) {
        ...
        if (mMailEditActivity.mZhuti.getText().toString().length() == 0) {
            ...
            if(MailManager.getInstance().mSelectUser != null)
             UserID = MailManager.getInstance().mSelectUser.mRecordid;
            else
                UserID = MailManager.getInstance().account.mRecordId;

            if (MailManager.getInstance().mSelectMailBox.isloacl) {
                String addres = "";
                if (MailManager.getInstance().mSelectUser.mRecordid
            ...
            if (MailManager.getInstance().account.iscloud == false) {
                ...
                for(int i = 0 ; i < mMailEditActivity.mMail.attachments.size() ; i++)
                {
                    if(mMailEditActivity.mMail.attachments.get(i).mRecordid.length() > 0 && mMailEditActivity.mMail.attachments.get(i).stata == Attachment.STATA_DOWNLOADFINISH)
                    {

                    }
                    else
                    {
                        AppUtils.showMessage(mMailEditActivity,mMailEditActivity.getString(R.string.mail_uploading));
                        return;
                    }
                }
                String To1 = measureMialPersonWordc(mMailEditActivity.mToPesrons, false);
                String Cc1 = measureMialPersonWordc(mMailEditActivity.mCcPesrons, false);
                String Bcc1 = measureMialPersonWordc(mMailEditActivity.mBccPesrons, false);
                String LocalCC1 = measureMialPersonWordc(mMailEditActivity.mLccPesrons, true);

                MailSend mailSend = new MailSend();
                if (issend) {
                    mailSend.send_state = "0";
                } else
                    mailSend.send_state = "4";
                mailSend.mail_box_id = MailManager.getInstance().mSelectMailBox.mRecordId;
                mailSend.subject = Subject;
                mailSend.content = mMailEditActivity.mContent.getText().toString();
                mailSend.from_address = MailManager.getInstance().mSelectMailBox.mFullAddress;
                mailSend.cc_address = Cc1;
                mailSend.to_address = To1;
                mailSend.bcc_address = Bcc1;
                if(mMailEditActivity.mAction == MailManager.ACTION_RESEND)
                mailSend.resent_address = To1;
                if (mMailEditActivity.mAction != MailManager.ACTION_EDIT) {
                    if (issend) {
                    ...
                    mailSend.state = String.valueOf(mMailEditActivity.mMail.state);
                }

                switch (mMailEditActivity.mAction) {
                    case MailManager.ACTION_NEW:
                        mailSend.mail_type = "1";
                ...
                if (mMailEditActivity.mAction == MailManager.ACTION_NEW || mMailEditActivity.mAction == MailManager.ACTION_EDIT) {
                    ...
                    mailSend.raw_mail_id = mMailEditActivity.mMail.mRecordId;
                    if (mMailEditActivity.mAction == MailManager.ACTION_RESEND) {
                        mailSend.raw_mail_type = "2";
                    } else {
                        mailSend.raw_mail_type = "3";
                    }
                }
                if (issend == false || mMailEditActivity.mAction == MailManager.ACTION_EDIT) {
                    mailSend.mail_user_id = mMailEditActivity.mMail.mRecordId;
                }
                ...
                if(mMailEditActivity.mMail.attachments.size() > 0)
                {
                    ...
                    {
                        try {
                            jsonObject.put(mMailEditActivity.mMail.attachments.get(i).mRecordid,mMailEditActivity.mMail.attachments.get(i).mailUploadJson);
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                    }
                ...
                {
                    mailSend.lable.put(lables.get(i).mId);
                }
                mMailEditActivity.waitDialog.show();
                MailAsks.sendMailc(mMailEditActivity, mMailEditHandler, mailSend,issend,mMailEditActivity.sentquery);
            }
    ...
    private String measureMialPersonWordc(ArrayList<MailContact> mPesrons, boolean locol) {
        String mail = "";
        for (int i = 0; i < mPesrons.size(); i++) {
            if (locol) {
                if(mail.length() > 0)
    ...
    private void onViewMeasure(MyLinearLayout mMyLinearLayout, ArrayList<MailContact> mPesrons, int type) {
        LayoutInflater mInflater = (LayoutInflater) mMailEditActivity.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        for (int i = 0; i < mPesrons.size(); i++) {
            if(mPesrons.get(i).getMailAddress().length() > 0)
            {
                boolean has = false;
                switch (type) {
                    case MailEditActivity.CONTENT_TYPE_SHOUJIAN:
    ...
    public long praseAttachmentSize(String size)
    {
        if(size.toLowerCase().endsWith("mb"))
        {
            double a = Double.valueOf(size.substring(0,size.indexOf(" ")));
        ...
        {
            if(size.contains(" "))
            return Long.valueOf(size.substring(0,size.indexOf(" ")));
            else
    ...
    {
        mMailEditActivity.sentquery = true;
        MailManager.getInstance().mSelectMailBox = null;
        for(int i = 0 ; i < MailManager.getInstance().allSendMailBoxs.size() ; i++)
        {
        ...
        if (mMailEditActivity.mCcLayer != null && mMailEditActivity.mBccLayer != null
            ...
            mMailEditActivity.mCcLayer.setVisibility(View.INVISIBLE);
            mMailEditActivity.mBccLayer.setVisibility(View.INVISIBLE);
            mMailEditActivity.mLccLayer.setVisibility(View.INVISIBLE);
            mMailEditActivity.mFajianTitle.setText(mMailEditActivity.getString(R.string.mail_fromtitle));
            mMailEditActivity.mFajian.setTextColor(Color.rgb(149, 149, 149));
            RelativeLayout.LayoutParams mFajianLayerParams = (RelativeLayout.LayoutParams) mMailEditActivity.mFajianLayer
                    .getLayoutParams();
            // mFajianLayerParams.removeRule(RelativeLayout.BELOW);
            mFajianLayerParams.addRule(RelativeLayout.BELOW, R.id.shoujianren_layer);
            mMailEditActivity.mFajianLayer.setLayoutParams(mFajianLayerParams);
        }
    ...
    public void setNomalSend()
    {
        ...
        mMailEditActivity.sentquery = false;
        mMailEditActivity.unshowall = false;
        mMailEditActivity.mCcLayer.setVisibility(View.VISIBLE);
        mMailEditActivity.mBccLayer.setVisibility(View.VISIBLE);
        mMailEditActivity.mLccLayer.setVisibility(View.VISIBLE);
        mMailEditActivity.mFajianTitle.setText(mMailEditActivity.getString(R.string.mail_fajianren1));
        mMailEditActivity.mFajian.setTextColor(Color.parseColor("#959595"));
        RelativeLayout.LayoutParams mFajianLayerParams = (RelativeLayout.LayoutParams) mMailEditActivity.mFajianLayer
                .getLayoutParams();
        // mFajianLayerParams.removeRule(RelativeLayout.BELOW);
        mFajianLayerParams.addRule(RelativeLayout.BELOW, R.id.bcc_layer);
        ...
        onEditclicklisten(MailEditActivity.CONTENT_TYPE_SHOUJIAN);
...
//        imm.showSoftInput(mMailEditActivity.mLcc, InputMethodManager.SHOW_FORCED);
    ...
    public View.OnClickListener setNomalMailListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            if(mMailEditActivity.sentquery == true)
            {
                setNomalSend();
                title.setText(mMailEditActivity.getString(R.string.mail_write_nomal));
                imgtitle.setImageResource(R.drawable.array_down);
                selectlayer.setVisibility(View.INVISIBLE);
                imgnomal.setVisibility(View.VISIBLE);
                imggrop.setVisibility(View.INVISIBLE);
            }

        }
    };

    public View.OnClickListener setGropMailListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            if(mMailEditActivity.sentquery == false)
            {
                setQuerySend();
                title.setText(mMailEditActivity.getString(R.string.mail_write_grop));
                imgnomal.setVisibility(View.INVISIBLE);
                imggrop.setVisibility(View.VISIBLE);
                imgtitle.setImageResource(R.drawable.array_down);
                selectlayer.setVisibility(View.INVISIBLE);
            }
    ...
    public View.OnClickListener selectSendTypeListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            if(selectlayer.getVisibility() == View.VISIBLE)
            {
                selectlayer.setVisibility(View.INVISIBLE);
                imgtitle.setImageResource(R.drawable.array_down);
            }
                ...
                imgtitle.setImageResource(R.drawable.array_up);
            }
        }
    };
    ...
    public View.OnClickListener mTabListener = new View.OnClickListener()
    {
        ...
        public void onClick(View v)
        {
            ...
            Intent intent = new Intent(mMailEditActivity, MailLableActivity.class);
            intent.putParcelableArrayListExtra("selects",lables);
            intent.putExtra("push",mMailEditActivity.sentquery);
            mMailEditActivity.startActivity(intent);
        }
    };

    public void setLable(Intent intent) {
        ArrayList<Select> selects = intent.getParcelableArrayListExtra("selects");
        lables.clear();
        if(selects != null)
        {
            for(int i = 0 ; i < selects.size() ; i++)
            {
                Select select1 = selects.get(i);
                if(select1 != null){
                    lables.add(select1);
                }
            }
    ...
    public void initLables() {
        if(mMailEditActivity.mMail != null)
        {
            String[] ja = mMailEditActivity.mMail.lables.split(",");
            for(int i = 0 ; i < ja.length ; i++)
            {
                ...
                if(mMailEditActivity.sentquery == true)
                {
                    select1  = MailManager.getInstance().getLable(ja[i]);
                }
                else
                {
                    select1  = MailManager.getInstance().getGLable(ja[i]);
                }

                if(select1 != null){
                    select.mId = select1.mId;
                    select.mName = select1.mName;
                    select.mColor = select1.mColor;
                    lables.add(select);
                }
