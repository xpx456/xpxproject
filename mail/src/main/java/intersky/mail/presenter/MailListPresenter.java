...
public class MailListPresenter implements Presenter {
    ...
    public void initView() {
        ...
        if(MailManager.getInstance().account.iscloud == false)
        ToolBarHelper.setTitle(mMailListActivity.mActionBar, getTitle(mMailListActivity.type1));
        else
            ToolBarHelper.setTitle(mMailListActivity.mActionBar, getTitlec(mMailListActivity.type2));
        ToolBarHelper.setRightBtn(mMailListActivity.mActionBar, mMailListActivity.mWriteMailListener, R.drawable.editnew);
    }
    ...
    public void startDelete(Mail mail)
    {
        if(mMailListActivity.buttomlyaer.getVisibility() == View.INVISIBLE)
        {
            ...
            ToolBarHelper.hidBack(mMailListActivity.mActionBar);
            ToolBarHelper.setRightBtnText(mMailListActivity.mActionBar, mMailListActivity.selectCancleListener, "取消");
            ToolBarHelper.setBackListenr(mMailListActivity.mActionBar, mMailListActivity.selectAllListener, "全选");
        }
