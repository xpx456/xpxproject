...
public class VideoPresenter implements Presenter {
    ...
    public void initView() {
        ...
        {
            TextView save = mVideoActivity.findViewById(R.id.save);
            save.setOnClickListener(mVideoActivity.mTakePhotoListener);
        }
    ...
    public void doItemClick(int position) {
        ...
        if(mVideoActivity.getIntent().getBooleanExtra("signal",false) == true)
        {
            File mFile = new File(mVideoItem.path);
            Attachment attachment = new Attachment("", mFile.getName(), mVideoItem.path, "", mFile.length(), mFile.length(), "");
            String action = mVideoActivity.getIntent().getAction();
            Intent mIntent = new Intent(action);
            mIntent.putExtra("attachment",attachment);
            mVideoActivity.sendBroadcast(mIntent);
